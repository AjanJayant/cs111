...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			proc_array[p].p_state = P_EMPTY; // Change implemented
							 // for the fourth subpa							 // rt
		}
		else
		{
	...
	case INT_SYS_NEWTHREAD:{
        	...
        	current->p_registers.reg_ea = new;
...
do_newthread(void (*start_function)(void),process_t *parent)
{
    ...
    proc_array[new].p_registers.reg_eip = (int)start_function;
...
do_fork(process_t *parent)
{
	...
	proc_array[child_id].p_registers = (*parent).p_registers;
	copy_stack(&proc_array[child_id], parent);
	
	proc_array[child_id].p_state = P_RUNNABLE;
	proc_array[child_id].p_registers.reg_eax = 0;
	return child_id; 
}
